...
(define (build)
  (unless (zero? (system "make compiler --quiet"))
    (error 'make "Could not build target.")))

(define (execute)
  (unless (zero? (system "./build/compiler > compiler.out"))
...
(define (get-string)
  (with-output-to-string
    (lambda ()
      (with-input-from-file "compiler.out"
        (lambda ()
          (let f ()
            (let ([c (read-char)])
              (cond
...
(define (compile-program expr)
  (let ([p (open-output-file "build/compiler.s" 'replace)])
    (parameterize ([compile-port p])
      (emit-program expr))
    (close-output-port p)))
